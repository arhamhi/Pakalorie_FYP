...
5. **CDX-001 ESCALATED (2026-07-02, session 2): the Supabase project is DELETED.** `prpvgqjeapmxyfroojfp.supabase.co` is NXDOMAIN (verified via 8.8.8.8) — every Supabase-backed feature (food logs, hydration, chat sessions, favorites, weight logs, avatars) is dead at the network level, not just insecure. Arham is deciding: fresh Supabase project (temporary bridge again) vs doing the Firestore migration now. If Firestore: schema shapes live in `src/types/database.ts`; suggest per-user subcollections (`users/{uid}/food_logs/{id}` etc.) so date-range queries need no composite indexes, plus `firestore.rules` additions mirroring the existing profile rule.
