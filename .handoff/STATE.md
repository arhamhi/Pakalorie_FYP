...
- **Backend (live on the VPS):** `POST /recognize` gained an optional `engine` field — `gemini` (default) | `yolo`. The `yolo` path runs our trained `best.onnx` (217 classes) server-side via `onnxruntime` in `app/services/yolo_recognition.py` (mirrors the MiDaS pattern in `depth.py`), returning the **identical `RecognitionResponse` shape** so the client never branches on engine. `app/services/recognition.py` dispatches by engine; the route validates `engine` via a `Literal` (422 on bad values).
  - **Real bug caught in review/testing:** `best.onnx` already includes a softmax op in the graph (output sums to 1, non-negative). The first cut re-softmaxed it, flattening the distribution to near-uniform (max prob 0.008 ≈ 1/217). Fixed: use the output as probabilities directly, with a guard that only re-softmaxes a future raw-logits re-export. After the fix: Haleem @ 0.57 with sensible alternatives (Dal Tadka, Biryani).
  - **Class list:** `ml/artifacts/checkpoints/class_names.json` (217, index-ordered) extracted from `best.onnx` metadata via `ml/scripts/extract_class_names.py` — that's the source ultralytics writes at export, guaranteed aligned with `output0`, so no torch/ultralytics install was needed. Cross-checked count vs `ml/reports/per_class_recall.csv`.
- **App:** Settings "Recognition Engine" selector (`app/(tabs)/settings.tsx`) — two tappable option rows (Gemini / YOLOv8) styled like the accent-color picker; the selected one gets an accent border + check-circle. (A first cut used an RN `<Switch>` but `flex:1` on the label squeezed it to zero width/untappable; commit `657565e` replaced it with the option-row pattern.) Persisted via `src/lib/preferences.ts` (`getRecognitionEngine`/`setRecognitionEngine`, default `gemini`). Threaded through `src/lib/api.ts` (`recognizeFood`/`recognizeAndGroundFood` take `engine`, sent as a multipart field) into `app/(tabs)/scan.tsx`; the results card shows which engine produced the label. Gemini stays default; the client-side Gemini fallback is unchanged.
- **Deployed:** VPS pulled to `main` @ `3274416`, rebuilt, and restarted. `best.onnx` + `class_names.json` copied into the host-mounted `backend/models/yolo/` (default env paths resolve to `/app/models/yolo/...`, no `.env` change). Steps in `backend/docs/DEPLOY.md` §4c.
  - **Verified from outside the VPS:** `curl .../recognize -F image=@test_food.webp -F engine=yolo` → `Haleem` @ 0.57; `engine=gemini` (correct MIME) → `Haleem` @ 0.98 (default unchanged). `/healthz` 200, container healthy.
- **Quality gates:** backend `pytest` 21 passed / 1 skipped (incl. real-inference + 503-on-missing-model + 503-on-corrupt-model), `ruff` clean, `npx tsc --noEmit` 0 errors. Reviewed by python-reviewer (2 HIGH — uncaught exceptions on corrupt model/labels now return 503 not 500 — fixed), typescript-reviewer + code-reviewer (no CRITICAL/HIGH; cheap MEDIUM/LOW hygiene applied).
- **Not done (intentional):** on-device inference stays P2 (would break Expo Go). `engine=gemini` forwarding the client MIME to Gemini is the same pre-existing behavior (honesty flag c below) — the app sets the MIME correctly so users never hit it. `ml/artifacts/checkpoints/last.pt` and `ml/scripts/colab_train_clean.py` were left untracked (pre-existing, not part of CDX-009; `last.pt` is redundant with the committed `best.pt`).
- **Commits (on `main`, pushed):** `22e0718` backend, `e753d7c` app, `3274416` Firebase config (firestore.rules/firebase.json/.firebaserc — were deployed-live but untracked).

## 2026-06-29 (Claude, Arham-driven) — P1 FINAL CLOSED OUT; all 6 modules verified live

- **All 6 graded modules (P1 Mid + P1 Final) verified LIVE end-to-end** by pushing the real `test_food.webp` through the deployed VPS: `POST /recognize` → "Haleem" @0.98; `POST /portion` → `medium` bucket (real ONNX); `POST /calories` → grounded breakdown (`gemini_grounded`, `why`, source rows); `GET /foods/search?q=biryani` → real `desi_v1` rows. `/healthz` ok.
- **YOLOv8 training RUN COMPLETE** (free Colab T4, 50 epochs, 224px): **top-1 0.5848 / top-5 0.8659 on 217 classes** (one 1-image class dropped in the split). Strong on well-sampled dishes (biryani 0.93, chai/samosa 1.00); 0-recall classes all have ≤5 val images — the documented 14.58x imbalance, not a model fault. `best.pt` (3.5 MB) + `best.onnx` (6.6 MB) + confusion matrix + per-class recall + qualitative + training curves committed under `ml/artifacts/checkpoints/` and `ml/reports/`; `ml/MODELCARD.md` updated with results + honest error analysis. (Commits `749cebe`, `81b9d41`.)
- **Real bug fixed + pushed:** GitHub had stale `ml/scripts/prepare_dataset.py` + `train.py` (≈105 lines behind local). `prepare_dataset.py` crashed in Colab with `UnboundLocalError: split_samples` — a loop variable `split_samples` shadowed the `split_samples()` function. Local fixed versions pushed (commit `8772094`). The repo is now the proven pipeline.
- **Firebase Firestore rules DEPLOYED** to project `pakalorie-fyp` via `firebase-tools` (Arham logged in as `arhamhfz77@gmail.com`). Fixes onboarding's "Missing or insufficient permissions" (the camera-permission step writes `users/{uid}`). Rule: a signed-in user reads/writes only their own profile doc. **`firestore.rules`, `firebase.json`, `.firebaserc` are on disk but still UNTRACKED** — commit them next session for version control.
- **Expo Go confirmed working on device:** app bundles clean (full Android export, 0 Metro errors) and runs; the Firebase-in-Expo-Go path holds (the `firebaseAuth.ts` deep-import workaround bundles fine). Login correctly persists across launches (session restore working) → authed users skip `(auth)` and route to `/onboarding/goal` if onboarding incomplete (by design, not a bug).
- **Docs:** `docs/SDS.md` §2.3 → "trained baseline complete" (with metrics), §2.4 MiDaS → "LIVE" (verified on the deployed VPS, was stale "deploy pending"). Added `docs/VIDEO_SCRIPT.md` (full showcase screencast script, defense-safe recognition framing).
- **Honesty flags still open:** (a) **streaks are display-only** — `streak_count` is rendered but never computed anywhere; don't demo it. (b) **Google sign-in on Android** is unconfigured (`googleSignInAndroidClientId` empty) — email/password is the working path. (c) `/recognize` forwards the client MIME to Gemini; the app sets it correctly so users never hit it (optional 1-line default-to-jpeg hardening).
