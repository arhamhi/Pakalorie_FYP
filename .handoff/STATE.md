...
**Last updated:** 2026-06-04 by Codex (CDX-005 prod compose + VPS DEPLOY runbook added and compose config validated; CDX-006 Kaggle datasets downloaded/validated and ML scaffold added; API is not live yet because the subdomain must be confirmed and the runbook still needs to be executed on the VPS)
**Next action owner:** Arham (confirm `api.srv987636.hstgr.cloud` or choose another API subdomain, then run the VPS deploy runbook or grant Codex SSH approval; provide own held-out food photos for YOLO qualitative tests); Codex (after deploy access/subdomain confirmation, verify HTTPS + n8n + private Postgres; next ML step is Colab YOLOv8n-cls training from `ml/notebooks/train_yolov8_cls.ipynb`); Claude (Phase 5 mobile wiring once the API is publicly reachable).
...
2. **Arham (inputs that unblock Codex):** (a) confirm `api.srv987636.hstgr.cloud` or choose another API subdomain, then run `backend/docs/DEPLOY.md` on the VPS or grant Codex SSH approval; (b) gather own food test photos (held-out YOLO test set); (c) keep smoke-testing Expo Go.
3. **Codex (priority order):** _Updated 2026-06-04 (Codex): CDX-002/003/004/008 are DONE and verified against the live VPS Postgres. CDX-005 now has prod compose + deploy runbook ready, but the public HTTPS acceptance check is not done until the runbook is executed on the VPS and `curl https://api.srv987636.hstgr.cloud/healthz` returns 200 from outside the box. CDX-006 dataset validation + training scaffold is done; training itself is still pending._ Next: execute/verify CDX-005 when Arham confirms the subdomain/access; run Colab `yolov8n-cls` training/eval for CDX-006; CDX-007 remains last/minimal.
