...
**Last updated:** 2026-06-03 by Claude (backend VERIFIED against live VPS Postgres: migrated + seeded 160 rows + every endpoint smoke-tested + live-DB integration suite added; found & fixed a Gemini thinking-token truncation bug)
...
- `/recognize` real-dish accuracy is unproven: the endpoint + server-side Gemini vision path work (verified shape + non-food "Unknown"), but no real desi food photo was available this session. Validate recognition quality with Arham's held-out photos (on-device, Phase 5).
